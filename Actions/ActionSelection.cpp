...
void ActionSelection::Execute()
{
	...
	if (CompSlct || ConnSlct)
	{
		...
		if (CompSlct)
		{
			pUI->PrintMsg("enter (el) to edit label, (ev) to edit value, (del) to delete, (c) to copy, (x) to cut or press ESC to escape");
		...
		{
			pUI->PrintMsg("enter (el) to edit label, (ec) to edit the connected components, (del) to delete or press ESC to escape");
			string ACT = pUI->GetSrting();
			pUI->ClearStatusBar();
